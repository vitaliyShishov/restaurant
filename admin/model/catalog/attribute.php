...
class ModelCatalogAttribute extends Model {
	...
	public function addAttribute($data) {
        ...
        if (!empty($data['allergens'])){
            foreach($data['allergens'] as $key => $allergen_id) {
                $this->db->query("INSERT INTO " . DB_PREFIX . "attribute_to_allergen SET allergen_group_id = '" . (int)$allergen_id . "', attribute_id = '" . (int)$attribute_id . "'");
            }
        }
    ...
    public function editAttribute($attribute_id, $data)
    {
        ...
        $this->db->query("DELETE FROM " . DB_PREFIX . "attribute_to_allergen WHERE attribute_id = '" . (int)$attribute_id . "'");
	...
	public function getAttribute($attribute_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "attribute a LEFT JOIN " . DB_PREFIX . "attribute_description ad ON (a.attribute_id = ad.attribute_id) LEFT JOIN ".DB_PREFIX."attribute_to_allergen a2a ON (a2a.attribute_id = ad.attribute_id) WHERE a.attribute_id = '" . (int)$attribute_id . "' AND ad.language_id = '" . (int)$this->config->get('config_language_id') . "'");
	...
	public function getAttributeAllergens($attribute_id) {
	    $query = $this->db->query("SELECT allergen_group_id FROM " . DB_PREFIX . "attribute_to_allergen WHERE attribute_id = '" . (int)$attribute_id . "'");

        if(!empty($query->rows)) {
            foreach($query->rows as $row) {
                $result[] = $row['allergen_group_id'];
            }
        } else {
            $result = array();
        }
